...
pub async fn run(server_url: &str, url: &str) -> anyhow::Result<()> {
    ...
    let keys = Arc::new(tokio::sync::Mutex::new(None::<SessionKeys>));
    ...
    let recv_to_ws_tx = to_ws_tx.clone();
                ...
                Ok(tokio_tungstenite::tungstenite::Message::Text(text)) => {
                    ...
                    match serde_json::from_str::<ServerMessage>(&text_str) {
                        ...
                        Ok(ServerMessage::AesKeys(ak)) => {
                            ...
                            let encrypted = pair_common::crypto::EncryptedKeys {
                                b64_output_key: ak.b64_output_key,
                                b64_input_key: ak.b64_input_key,
                                iv_count: ak.iv_count,
                                max_iv_count: ak.max_iv_count,
                            };
                            match SessionKeys::extract_keys(&bootstrap, &encrypted) {
                                Ok(session_keys) => {
                        ...
                        Ok(ServerMessage::Chat(chat)) => {
                            println!("\r\x1b[K[pair chat] {}: {}", chat.from, chat.text);
                        }
